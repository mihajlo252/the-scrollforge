...
import { advancementContext, choiceIsValid, commitLevelUp, picksCost, type AdvancementChoice } from "../../../utilities/daggerheartLevelUp";
...
const CONNECTION_PROMPTS = ["Who in the party do you trust most, and why?", "Who do you owe something to?"];
		...
		<div className={styles.wrap}>
			...
			<div className={styles.panel}>
										...
										{allClasses().map((c) => {
											...
											return (
												...
												>
													...
													<span className={styles.pickMeta}>
														EV {c.startingEvasion} · HP {c.startingHitPoints}
													</span>
												</button>
											);
										})}
									...
									<div className={styles.pickGrid}>
										{subclassesForClass(draft.class).map((s) => (
											<button
												...
												type="button"
												className={styles.pick}
												data-active={draft.subclass === s.name}
												onClick={() => set({ subclass: s.name })}
											>
												<span className={styles.pickName}>{s.name}</span>
												<span className={styles.pickMeta}>
													{s.spellcastTrait ? `Spellcast: ${s.spellcastTrait}` : "Martial"}
												</span>
											</button>
										))}
									</div>
									...
									{cls && (
										<>
											...
											<div className={styles.detailMeta}>
												{cls.domains.map((d) => Capitalize(d)).join(" & ")} · Evasion {cls.startingEvasion} · HP{" "}
											...
											{subclassCfg && (
												<>
													<div className={styles.detailSection}>
														{subclassCfg.name} ·{" "}
														{subclassCfg.spellcastTrait ? `Spellcast ${subclassCfg.spellcastTrait}` : "Martial"}
													</div>
													{subclassCfg.foundation.features.map((f) => (
														<FeatureLine key={`f-${f.name}`} f={f} />
													))}
												</>
											)}
										</>
									...
									<div className={styles.pickGrid}>
										{allAncestries().map((a) => (
											<button
												...
												type="button"
												className={styles.pick}
												data-active={draft.ancestry === a.name}
												onClick={() => set({ ancestry: a.name })}
											>
									...
									<div className={styles.pickGrid}>
										{allCommunities().map((c) => (
											<button
												...
												type="button"
												className={styles.pick}
												data-active={draft.community === c.name}
												onClick={() => set({ community: c.name })}
											>
								...
								<aside className={styles.details}>
									{ancestryCfg && (
										<>
											...
											{ancestryCfg.features.map((f) => (
												<FeatureLine key={`a-${f.name}`} f={f} />
											))}
										</>
									)}
									{communityCfg && (
										<>
											...
											{communityCfg.features.map((f) => (
												<FeatureLine key={`c-${f.name}`} f={f} />
											))}
										</>
									)}
								</aside>
					...
					<>
						<div className={styles.panelHead}>
							...
							<div className={styles.panelHint}>
								Assign the standard array: +2, +1, +1, 0, 0, −1. {spellTrait ? `Your Spellcast trait is ${spellTrait}.` : ""}
							</div>
						</div>
						<div className={styles.body}>
							<div className={styles.traitAssign}>
								{TRAIT_NAMES.map((t) => (
									<div
										key={t}
										className={styles.traitAssignRow}
										data-spellcast={!!spellTrait && spellTrait.toUpperCase() === t.toUpperCase()}
									>
							...
							<span className={styles.poolNote}>
								Each value comes from the fixed array — choosing one automatically swaps it with whichever trait held it, so you
								always have one +2, two +1, two 0, and one −1.
							</span>
						</div>
					</>
				)}
				...
				{current === "equipment" && (
					<>
						<div className={styles.panelHead}>
							...
							<div className={styles.panelHint}>
								Choose from the weapon and armor catalog (tier {gearTier} or lower). You can refine these later on the sheet.
							</div>
						</div>
						<div className={styles.body}>
									...
									{primaryWeapons()
										...
										.map((w) => (
											<option key={w.id} value={w.id}>
												{w.name} · T{w.tier} · {formatWeaponDamage(w)} {titleWords(w.trait)}
											</option>
										))}
								</select>
							</Field>
									...
									{allArmors()
										...
										.map((a) => (
											<option key={a.id} value={a.id}>
												{a.name} · T{a.tier} · Score {a.baseScore} · {a.baseMajorThreshold}/{a.baseSevereThreshold}
											</option>
										))}
								</select>
							</Field>
							...
							{draft.background.map((entry, i) => (
								<div key={i}>
									...
									<textarea
										...
										value={entry.a}
										onChange={(e) =>
											set({ background: draft.background.map((x, idx) => (idx === i ? { ...x, a: e.target.value } : x)) })
										}
									/>
								</div>
							))}
								...
								<div key={i} className={styles.formGrid2}>
									<Field label={`Experience ${i + 1}`}>
										<input
											...
											onChange={(e) =>
												set({
													experiences: draft.experiences.map((y, idx) => (idx === i ? { ...y, name: e.target.value } : y)),
												})
											}
										/>
									</Field>
									<Field label="Bonus">
										<input
											...
											onChange={(e) =>
												set({
													experiences: draft.experiences.map((y, idx) =>
														idx === i ? { ...y, bonus: parseInt(e.target.value) || 0 } : y,
													),
												})
											}
										/>
				...
				{current === "domains" && (
					<>
						<div className={styles.panelHead}>
							...
							<div className={styles.panelHint}>
								Choose up to {maxCards} cards (level {draft.level} or lower) from your {cls?.name} domains (
								{classDomains.map(Capitalize).join(" & ")}). Selected {draft.domainCards.length}/{maxCards}.
							</div>
						</div>
						<div className={styles.body}>
							{groupCardsByLevel(
								CATALOG.filter((c) => c.level <= draft.level && classDomains.some((d) => d.toUpperCase() === c.domain.toUpperCase())),
							).map(([lvl, cards]) => (
									...
									{cards.map((card) => {
										...
										return (
											<DomainCardView key={card.name} card={card}>
												<button
													type="button"
													className={`button ${active ? "button-secondary" : "button-primary"} short`}
				...
				{current === "advancements" && (
					<>
						<div className={styles.panelHead}>
							...
							<div className={styles.panelHint}>
								Starting at level {draft.level} grants every level-up along the way: choose advancements worth 2 points for each
								level. Tier entries (levels 2, 5, and 8) automatically add +1 Proficiency and a new Experience, and damage thresholds
								rise by 1 every level.
							</div>
						</div>
						<div className={styles.body}>
								...
								<div key={lvl}>
									...
									{locked ? (
										...
										<>
											...
											<AdvancementPicker
												ctx={ctx}
												selected={advPicks[lvl] ?? {}}
												onChange={(next) => setLevelPicks(lvl, next)}
												baseClass={draft.class}
											/>
										</>
									)}
								</div>
							...
							{draft.connections.map((entry, i) => (
								<div key={i}>
									...
									<textarea
										...
										value={entry.a}
										onChange={(e) =>
											set({ connections: draft.connections.map((x, idx) => (idx === i ? { ...x, a: e.target.value } : x)) })
										}
									/>
								</div>
							))}
				...
				<div className={styles.nav}>
					...
					<button type="button" className="button button-ghost" onClick={() =>  step > 0 ? setStep(step - 1) : toast({style: "", message: "Don't burn yourself!"})}>
						<Icon name={step > 0 ? "back" : "flame"} size={step > 0 ? 13 : 16} /> {step > 0 && "Back"}
					</button>
					{step < steps.length - 1 ? (
