...
const patchCharacter = (state: any, patch: Partial<Character>) => {
	localStorage.setItem("character", JSON.stringify({ state: { ...state, character: { ...state.character, ...patch } }, version: 1 }));
};
					...
					<div className={styles.castHeader}>
						<div className={styles.spellcasting}>
							...
							<div className={styles.castInfo}>
								<select
									className={`select ${styles.castSelect}`}
									value={slots.ability}
									onChange={(e) => persistSlots({ ...slots, ability: e.target.value })}
								>
									{ABILITIES.map((a) => (
						...
						<div className={styles.spellsTopActions}>
				...
				<Frame classes="card">
					...
					<div className={styles.spellListBody}>
						...
						{[9, 8, 7, 6, 5, 4, 3, 2, 1, 0].map((lv) => {
							const group = filtered.map((s) => ({ s, i: spells.indexOf(s) })).filter(({ s }) => (s.level ?? 0) === lv);
							...
							return (
								<div key={lv} style={{ marginBottom: 10 }}>
									...
									{group.map(({ s, i }) => (
										...
										>
											<span
												className={styles.spellItemBar}
												style={{
													background: (s.level ?? 0) === 0 ? "var(--ink-faint)" : "var(--gold)",
													opacity: s.prepared ? 1 : 0.35,
												}}
											/>
						...
						<div className={styles.spellDetail}>
							<div className={styles.spellDetailHead}>
								...
								<div className={styles.spellActions}>
									...
									<ConfirmButton
										className="button button-secondary short"
										aria-label="Delete spell"
										title="Delete spell?"
										message="Remove this spell? This can't be undone."
										onConfirm={() => deleteSpell(selected)}
									>
							...
							<div className={styles.spellStatStrip}>
								{[
									...
									<div key={label} className={styles.spellStat}>
										<div className="caps" style={{ fontSize: 9, marginBottom: 2 }}>
											{label}
										</div>
					...
					<Frame classes={styles.addFrame}>
						...
						<div className={styles.formRow}>
							<input
								...
								onChange={(e) => setField("castingTime", e.target.value)}
							/>
						...
						<div className={styles.formRow}>
							<button type="submit" className="button button-accent stretch">
								{editIndex === null ? "Add" : "Save"}
							</button>
							<button type="button" className="button button-secondary stretch" onClick={() => setShowForm(false)}>
								Close
							</button>
						</div>
					</Frame>
			...
			<Popup closerFunc={setShowSlotEdit} toggle={showSlotEdit}>
				<Frame classes={`${styles.addFrame} ${styles.slotsPopup}`}>
					...
					<div className={styles.formRow}>
						<button
							type="button"
							className="button button-accent stretch"
							onClick={() => {
								persistSlots(slots);
						...
						<button type="button" className="button button-secondary stretch" onClick={() => setShowSlotEdit(false)}>
							Close
						</button>
					</div>
				</Frame>
